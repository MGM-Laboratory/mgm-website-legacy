...
export const DEFAULT_PROJECT_PAGE_SIZE = PROJECT_PAGE_SIZES[0];
...
/** Same page/size as `?page=`/`?per=`, but keeps whatever `q`/`category` is already set. */
function gridHref(baseParams: URLSearchParams, page: number, perPage: number) {
  const params = new URLSearchParams(baseParams);
  params.set("page", String(page));
  params.set("per", String(perPage));
  return `/projects?${params.toString()}#projects`;
}
  ...
  return (
    <div className="mt-16 flex flex-col gap-6 border-t border-[var(--line)] pt-8 lg:flex-row lg:items-center lg:justify-between">
      <p className="font-mono text-[11px] tracking-[0.14em] text-[var(--ink-3)] uppercase dark:text-white/45">
        {first}
        {"-"}
        {last} of {total} projects
      </p>

      {totalPages > 1 ? (
        <nav aria-label="Project pages" className="flex flex-wrap items-center gap-2">
          {page > 1 ? (
            <Link
              aria-label="Previous page"
              className={`${BUTTON_BASE} ${BUTTON_IDLE}`}
              href={gridHref(baseParams, page - 1, perPage)}
            >
              Previous
            </Link>
          ) : (
          ...
          {markers.map((marker) =>
            typeof marker === "number" ? (
              <Link
                ...
                href={gridHref(baseParams, marker, perPage)}
                key={`page-${marker}`}
              >
                {marker}
              </Link>
        ...
        <div className="flex items-center gap-2">
          {PROJECT_PAGE_SIZES.map((size) =>
            size === perPage ? (
              ...
              <Link
                aria-label={`Show ${size} projects per page`}
                className={`${BUTTON_BASE} ${BUTTON_IDLE} h-9 min-w-9`}
                href={gridHref(baseParams, 1, size)}
                key={`size-${size}`}
              >
                {size}
              </Link>
