...
/**
 * Drives the server-side article search through the URL: typing debounces
 * into `/articles?q=...` so results stay server-rendered and shareable.
 */
export function ArticleSearchBox({ initialQuery = "" }: { initialQuery?: string }) {
  ...
  const navigate = (next: string) => {
    ...
    router.replace(trimmed ? `/articles?q=${encodeURIComponent(trimmed)}` : "/articles");
  };
