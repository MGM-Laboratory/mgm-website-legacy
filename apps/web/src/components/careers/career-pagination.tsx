...
export const JOB_PAGE_SIZES = [9, 18, 45] as const;
export const DEFAULT_JOB_PAGE_SIZE = JOB_PAGE_SIZES[0];
...
export function CareerPagination({
  page,
  perPage,
  total,
  hrefFor,
  ...
  perPage: number;
  total: number;
  /** Builds a page URL that keeps the active filters attached. */
  hrefFor: (page: number, per: number) => string;
}) {
  ...
  return (
    <div className="mt-16 flex flex-col gap-6 border-t border-[var(--line)] pt-8 lg:flex-row lg:items-center lg:justify-between">
      <p className="font-mono text-[11px] tracking-[0.14em] text-[var(--ink-3)] uppercase dark:text-white/45">
        {first}
        {"-"}
        {last} of {total} role{total === 1 ? "" : "s"}
      </p>

      {totalPages > 1 ? (
        <nav aria-label="Role pages" className="flex flex-wrap items-center gap-2">
          {page > 1 ? (
            <Link
              aria-label="Previous page"
              className={`${BUTTON_BASE} ${BUTTON_IDLE}`}
              href={hrefFor(page - 1, perPage)}
            >
              Previous
            </Link>
          ) : (
        ...
        <div className="flex items-center gap-2">
          {JOB_PAGE_SIZES.map((size) =>
            size === perPage ? (
              ...
              <Link
                aria-label={`Show ${size} roles per page`}
