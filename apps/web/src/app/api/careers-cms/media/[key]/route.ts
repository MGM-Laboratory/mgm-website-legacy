...
// Job description images stream from the API's signed-URL redirect the same
// way article covers do; the key allowlist lives on the API side.
export async function GET(request: Request, { params }: { params: Promise<{ key: string }> }) {
  const { key } = await params;

  const response = await cmsApi(`/cms/jobs/media/${encodeURIComponent(key)}`, {
    redirect: "manual",
  });
